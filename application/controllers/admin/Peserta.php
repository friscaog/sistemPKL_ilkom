...
class Peserta extends CI_Controller {
	...
	public function verifikasi_tahapan()
	{
		$this->load->helper('output_ajax');
		$this->load->model('peserta_model');
		...
		{	
			...
			if (is_array($ids))
			{
				foreach ($ids as $id) 
				{
					$result &= $this->peserta_model->tahapan_lanjutkan($id);
				}
			}
			// Operasi satu id.
			else
			{
				$result = $this->peserta_model->tahapan_lanjutkan($ids);
			}			
		}
		catch (Exception $e)
		{
		...
		{
			$result = $this->_get_list_peserta(TRUE);
			send_ajax_output(200, 'Gagal memproses sebagian atau seluruh data peserta yang dipilih', $result);
			return;	
		}
		// Gagal memproses operasi satu id.
		else
		{
			send_ajax_output(400, 'Tahapan peserta saat ini tidak valid');
			return;
		}
	}

	public function ubah_tahapan()
	{
		$this->load->helper('output_ajax');
		$this->load->model('peserta_model');
		$this->load->language('cs_pkl_error');
		...
		try
		{
			$result = $this->peserta_model->update_tahapan($id, $this->input->post('tahapan'));
		}
		catch (Exception $e)
		{
			log_message('error', $e->getMessage());
		...
		{
			// Ambil data detail.
			$data = $this->_get_data_detail($id);
			send_ajax_output(200, 'Ok', $data);
			return;
		}
		else
		{
			send_ajax_output(400, 'Tahapan tidak valid');
			return;
		}
	}

	public function ubah_data_pendaftaran()
	{
		$this->load->helper('output_ajax');
		$this->load->model('peserta_model');
		$this->load->language('cs_pkl_error');
	...
	public function ubah_laporan_judul()
	{
		$this->load->helper('output_ajax');
		$this->load->model('peserta_model');
		$this->load->language('cs_pkl_error');
		...
		try
		{
			...
			$this->peserta_model->update_laporan_judul(
				$id, 
				$this->input->post('laporan_judul')
				);
	...
	public function delete()
	{
		...
		try
		{
			$this->peserta_model->delete($this->input->post('id'));
	...
	private function _get_data_detail($id)
	{
		...
		$this->load->model(array('peserta_model', 'periode_model', 'dosen_model', 'tempat_model'));


		// Ambil data peserta tertentu.
		...
		foreach ($data_periode as &$value) {
			$value['per_nama'] = $value['per_nama'].' (TA '.$value['per_tahun'].' semester '.$value['per_semester'].')';
		}


		// Ambil seluruh data dosen.

		$data_dosen = $this->dosen_model->get();


		// Ambil seluruh data tempat pkl.
		 
		$data_tempat_pkl = $this->tempat_model->getActive();


		return array(
			'peserta' => $data_peserta_detail, 
			'periode' => $data_periode, 
			'dosen' => $data_dosen, 
			'tempat_pkl' => $data_tempat_pkl
			);
	}
